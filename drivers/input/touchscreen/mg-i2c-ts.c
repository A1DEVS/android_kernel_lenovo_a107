...
struct mg_data{
	__u16 	x[CFG_MAX_TOUCH_POINTS], y[CFG_MAX_TOUCH_POINTS], w[CFG_MAX_TOUCH_POINTS], p, id[CFG_MAX_TOUCH_POINTS];
...
/*<-- LH_SWRD_CL1_Henry@2012.3.3 define constant for kernel3.0  -->*/	
/* -- how many points of touch: 2, but our driver receive 2 touch point seperately in  mg_i2c_work() -- */

#define PRESS_MAX       0xFF
#define MG_PRESS	0x7F
int actual_touch_point_number = 0;

static inline int mg_mtreport(struct mg_data *mg, int touch_num)
{
#if 0	//Henry Li: kernel2.6.32
...
#else
/*<-- LH_SWRD_CL1_Henry@2012.3.3 change to ICS (kernel3.0) format -->*/	
int i=0;
int uppoint = 0;
for (i  = 0; i < touch_num; i++){
	if(vertical){

		     if((mg->y[i] > CAP_Y_CORD)||(mg->x[i] > CAP_X_CORD))	
			 	
	            return EIO;	 
		} else {

   			  if((mg->y[i] > CAP_X_CORD)||(mg->x[i] > CAP_Y_CORD))	
	...
	for (i  = 0; i < touch_num; i++)
	{
		...
		input_mt_slot(mg->dev, mg->id[i]);
		
		
		if (mg->w[i])		//touch down
		{
		    //  input_mt_slot(mg->dev, touch_num);
			input_mt_report_slot_state(mg->dev, MT_TOOL_FINGER, true);
			//input_report_abs(mg->dev, ABS_MT_TOUCH_MAJOR, MG_PRESS);
			input_report_abs(mg->dev, ABS_MT_POSITION_X, (mg->x[i] ));
			input_report_abs(mg->dev, ABS_MT_POSITION_Y, (mg->y[i] ));
			//input_report_abs(mg->dev, ABS_MT_PRESSURE,    200);
			input_report_key(mg->dev, BTN_TOUCH, 1);
	
			
		}
              else       //up,not need to report coordinate of x y
		{
			uppoint++;
			//input_mt_slot(mg->dev, mg->id[i]);
			input_mt_report_slot_state(mg->dev, MT_TOOL_FINGER, false);
			input_report_key(mg->dev, BTN_TOUCH, 0);
		}
	}


	/* Henry Li:  need or not ?? 
	if (actual_touch_point_number == 2)
		input_report_key(mg->dev, BTN_TOUCH, 0);
	else
		input_report_key(mg->dev, BTN_TOUCH, (2 > 0));
	*/
	//input_report_key(mg->dev, BTN_TOUCH, (2 > 0));
	//input_sync(mg->dev);		//it's in mg-i2c-work already
	return 0;
...
//				printk("=******MORGAN_TOUCH******* touch_led  on \n");
		...
		case 0:
			...
			input_report_key(mg->dev, BTN_TOUCH, 0);
				...
				printk("BTN_TOUCH11\n"); 
			#endif
			input_sync(mg->dev);input_report_key(mg->dev, BTN_TOUCH, (2 > 0));
			#ifdef RANDY_DEBUG	
				printk("sync\n"); 
			#endif
			break;
	...
	else
	{
		...
		if(vertical){
			 ...
			 mg->x[0] = COORD_INTERPRET(read_buf[MG_POS_X_HI], read_buf[MG_POS_X_LOW]);
			 mg->y[0] = CAP_MAX_Y - COORD_INTERPRET(read_buf[MG_POS_Y_HI], read_buf[MG_POS_Y_LOW]);

                         } else {

			  mg->y[0] = COORD_INTERPRET(read_buf[MG_POS_X_HI], read_buf[MG_POS_X_LOW]);
			  mg->x[0] = COORD_INTERPRET(read_buf[MG_POS_Y_HI], read_buf[MG_POS_Y_LOW]);
		}
			
			mg->w[0] = read_buf[MG_STATUS];
			mg->id[0] = read_buf[MG_CONTACT_ID]-1;
			//actual_touch_point_number = 1;		//Henry Li: first point
			error=mg_mtreport(mg,1);
		}
		if(read_buf[ACTUAL_TOUCH_POINTS] == 2) 
		{
			...
			mg->id[0] = read_buf[MG_CONTACT_ID]-1;
				...
				if(vertical){
		       	...
		       	mg->x[1] =	COORD_INTERPRET(read_buf[MG_POS_X_HI2], read_buf[MG_POS_X_LOW2]);
			      mg->y[1] = CAP_MAX_Y - COORD_INTERPRET(read_buf[MG_POS_Y_HI2], read_buf[MG_POS_Y_LOW2]);
			
                    } else {
                    
			       mg->y[1] =	COORD_INTERPRET(read_buf[MG_POS_X_HI2], read_buf[MG_POS_X_LOW2]);
			      mg->x[1] = COORD_INTERPRET(read_buf[MG_POS_Y_HI2], read_buf[MG_POS_Y_LOW2]);
		     }
			
			mg->w[1] = read_buf[MG_STATUS2];
			mg->id[1] = read_buf[MG_CONTACT_ID2]-1;
			actual_touch_point_number = 2;		//Henry Li: second point
			error=mg_mtreport(mg,2);
		}
		if (error == 0)
		input_sync(mg->dev);
	}
...
/*<-- LH_SWRD_CL1_richard@20111219  add to selection*/
sel_touchpanel(struct i2c_client *client)
{
	int sel = 0;
	char buf1[5] = {0x97, 0x0, 0x0, 0x0, 0x0};
	int err;
	char buf[5]={0xDE, 0xDD, 0x00, 0x00, 0x00};

/* <-- LH_SWRD_CL1_Henry@2012.6.7 reset touch in touch initialization  */	
	...
	sel = i2c_master_send(client, buf, 5);
    	if (!sel) 
		printk("[Driver] i2c_master_send fail, ret=%d \n", sel);
	printk("[Driver] i2c_master_send , ret=%d \n", sel);

	sel = i2c_master_recv(client, read_buf, 5);
	if(!sel)
		printk("[Driver] i2c_master_recv fail, ret=%d \n", sel);
	printk("[Driver] i2c_master_recv , ret=%d \n", sel);

	if(sel>0) {
		err = i2c_master_send(client, buf1, 5);
		if(err<0)
			printk("[Driver] i2c_master_send fail, err=%d \n", err);
	}	
...
#if 1
			printk("[MG_TP]: Insert the sel_touchpanel!\n");
			int ret;
			ret=sel_touchpanel(client);
			if(ret < 0)
				goto probe_out;
			printk("[MG_TP]: MG_TP was selected!\n");
#endif
/*Richard_cheng@20111217--*/
...
//&*&*&*JJ1	
	...
	//set_bit(ABS_MT_PRESSURE, input_dev->absbit);
		//Henry Li: kernel3.0 format
	input_mt_init_slots(input_dev, CFG_MAX_TOUCH_POINTS);
	//set_bit(ABS_MT_WIDTH_MAJOR, input_dev->absbit);
	
	//input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, 200, 0, 0);
...
/*++Richard_cheng@20111219 for err*/
probe_out:
		printk("[MG_TP]:mg_touch was not used!\n ");
		i2c_set_clientdata(client, NULL);
		misc_deregister(&mg->firmware);
/*Richard_cheng@20111219 for err--*/
exit_kfree:
	kfree(mg);
	return err;
